<template>
  <div class="details box">
    <div class="box-heading justify-content-between">
      ...
      <div>
        <button
          v-if="modify" class="btn btn-yellow d-flex justify-content-center align-items-center"
          @click.prevent="update()"
        >
          <loading-circle
            :loading="btnLoading" :small="true"
            :color="'#ffffff'" class="mr-2"
          />
          {{ __('Modify plan') }}
        </button>

        <button
          v-else class="btn btn-orange d-flex justify-content-center align-items-center blob orange"
          @click.prevent="update()"
        >
          <loading-circle
            :loading="btnLoading" :small="true"
    ...
    <div
      v-if="available && !loading" class="box-body"
    ...
    >
      <div class="body-wrap flex-column">
        ...
        />
        <fare-breakup
          :timeslot="activeTimeslot" :travellers="tempTravellers"
          :currency="currency"
        />
      </div>
    </div>
...
import {NO_MORE_SEATS} from '../../bootstrap/notify-messages'
...
export default {
  ...
  methods: {
    ...
    onInputTravellers (travellers) {
      const activeTravellers = travellers
        .filter(e => {
      ...
      if (activeTravellers.length) {
        ...
        if (!available) {
          this.$notification.show(NO_MORE_SEATS)
